...
parks_buffer <- st_transform(parks_buffer, st_crs(census_tracts_pa))
...
# 确保 CRS 一致
census_access <- st_transform(census_access, 3857)   # 投影为3857
parks_proj <- st_transform(parks, st_crs(census_access))

# 给 census tracts 添加收入组
census_access <- census_access %>%
  mutate(
...
ggplot() +
  # Census tracts
  geom_sf(data = census_access, aes(fill = income_group), color = "white", size = 0.2) +
  
  # 公园点
  geom_sf(data = parks_proj, color = "#d7191c", size = 1.5, alpha = 0.8) +
  
  # 配色
  scale_fill_manual(
    name = "Income Group",
    values = c("Low Income" = "#4575b4", "High Income" = "#fee090")
  ) +
  
  labs(
    title = "Philadelphia Census Tracts by Income and Park Locations",
    subtitle = "Census tracts colored by income group; red dots = parks",
